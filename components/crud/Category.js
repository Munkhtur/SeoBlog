...
import {create, getCategories, removeCat} from '../../actions/category'
...
const Category = ()=>{
    ...
    const showCategories = () =>{
        return categories.map((c, i)=>{
            return (
            <button onDoubleClick={()=> deleteConfirm(c.slug)} title ="Double click to delete" key={i} className="btn btn-outline-primary mx-1 mt-3">{c.name}</button>
            )
        })
    }
    ...
    const deleteCategory=(slug)=>{
        removeCat(slug, token).then(data=>{
            if(data.error){
                console.log(data.error)
            }else{
